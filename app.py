...
def render_tab(market_key: str, label: str, emoji: str):
    """단일 시장 탭 렌더링."""
    st.info("""
    **💡 P/CF (주가현금흐름비율) 핵심 가이드**
    *   **의미**: 기업이 실제로 번 돈(현금) 대비 주가가 얼마나 싼지 보여줍니다. (PER보다 신뢰도가 높습니다)
    *   **보는 법**: 📉 **수치가 낮을수록 저평가(싸다)** 입니다!
        *   🟢 **10 이하**: 저평가 (돈 잘 버는데 주가가 쌉니다)
        *   🔴 **20 이상**: 고평가 (돈 버는 것에 비해 주가가 비쌉니다)
    """)
